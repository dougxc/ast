...
//File: TypeDefs.h
//Author: Doug Simon
//Purpose: provide an enumerated type for SPARC8 assembly instruction opcodes and an
//	enumerated type for basic blocks of these instructions. Simple functions are provided
//	to convert between the two as well as to convert each to/from a string

#ifndef _TYPEDEFS_
#define _TYPEDEFS_

#define MAX_OPCODE_LEN 7	// the maximum string length of an opcode

// Define basic block  types
enum bbType {
	cBranch,       // a conditional branch
	call,        // branch on a call
	uBranch,      // an unconditional branch
	nway,	     //  jump on a register value
	ret,
	fall,        // a fall through basic block
	intNode,     // this is actually an interval node
	other
};
